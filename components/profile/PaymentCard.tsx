    ...
    return (
        <div className="flex items-center justify-between h-20 border border-[#828282] rounded-[10px] py-4 px-4  max-md:py-2 max-md:px-4  mr-2.5">
            <div className="flex items-center gap-3.5 ">
                <div>
                    {
                        card.cardType === "visa" ? (
                            ...
                            </svg>) : card.cardType === "mastercard" ? (
                                <svg width="71" height="51" viewBox="0 0 71 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect x="0.5" y="0.5" width="70" height="50" rx="7.5" stroke="#D9D9D9" />
                                    <circle cx="27.5" cy="25.5" r="11.5" fill="#F00009" />
                                    <circle cx="43.5" cy="25.5" r="11.5" fill="#F8A000" fillOpacity="0.7" />
                                </svg>
                ...
                <div >
                    <p className="font-medium text-balance text-[#5B5B5B] leading-[100%] mb-2 text-xs">
                        {card.lastFourDigits} ilə bitən {card.cardType === "visa" ? "Visa" : card.cardType === "mastercard" ? "MasterCard" : card.cardType === "other" ? "Digər Kart" : ""}
                    </p>
            ...
            <div className="flex items-center gap-5 max-md:gap-3">
                ...
                {card.isDefault ? (
                    ...
                    <button className="cursor-pointer font-medium text-xs text-[#003CFF] max-md:text-center" onClick={() => handleSetDefaultCard(card.id)}>
                        Əsas kart kimi təyin et
                    </button>
                )}

                <button className="group cursor-pointer" onClick={() => handleDeleteCard(card.id)}>
                    <HiOutlineTrash
